...
def get_frequent_users(problem_attempts, min_attempts):
    user_problem_attempts = problem_attempts.groupby(['user_id', 'question_idx']).agg({
        'user_name': 'first',
        'attempt_count': 'sum'
    }).reset_index()
    ...
    frequent_users = user_problem_attempts.groupby('user_id').filter(
        lambda x: all(x['attempt_count'] >= min_attempts) and len(x) == len(all_problems)
    )
    
    result = frequent_users.groupby('user_id').agg({
        'user_name': 'first'
    }).reset_index()
...
st.set_page_config(page_title="Comprehensive Problem Dashboard", page_icon="📊", layout="wide")
...
if selected_topic:
    ...
    st.subheader(f"{config.TOPICS[selected_topic]['title']} 문제별 풀이 현황", divider='rainbow')
    ...
    if not problem_attempts.empty:
        ...
        if start_date and end_date:
            ...
            st.write(f"데이터 기간: 전체 (첫 시도: {first_attempt.strftime('%Y-%m-%d')}, 마지막 시도: {last_attempt.strftime('%Y-%m-%d')})")
        
        # 문제 idx 선택 (전체 옵션 추가)
        question_idx_list = ["전체"] + sorted(problem_attempts['question_idx'].unique().tolist())
        ...
        if selected_idx == "전체":
            ...
            min_attempts = st.number_input("각 문제당 최소 시도 횟수", min_value=1, value=3, step=1)
            ...
            # 선택된 문제의 데이터 필터링
            filtered_data = problem_attempts[problem_attempts['question_idx'] == selected_idx]
        ...
        st.info(f"{config.TOPICS[selected_topic]['title']} 주제에 대한 선택한 기간의 풀이 데이터가 없습니다.")
