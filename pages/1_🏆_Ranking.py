...
if ranking_type == "전체 랭킹":
    ...
    for i in range(min(3, len(top_3))):
        with columns[i]:
            if i < len(top_3):
                ...
                accuracy = (user['correct'] / total_attempts * 100) if total_attempts > 0 else 0
                st.metric(
    ...
    with col1:
        school_filter = st.selectbox("소속 필터", ["전체"] + list(ranking_data['school'].unique()))
    with col2:
        team_filter = st.selectbox("팀 필터", ["전체"] + list(ranking_data['team'].unique()))
    ...
    # 랭킹 표시
    st.dataframe(ranking_data[['rank', 'name', 'school', 'team', 'points', 'correct', 'incorrect']])
