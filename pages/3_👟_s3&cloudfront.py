...
# 현재 홈워크 주제 설정
CURRENT_TOPIC = "s3_cloudfront"
TITLE = "👟 S3 & CloudFront Homework"
...
st.set_page_config(page_title="S3 & CloudFront Homework", page_icon="👟", layout="wide")
...
questions = load_homework_questions(topic_config["file"])
...
st.subheader(f"문제 {current_q + 1} / {st.session_state.total_questions}")
...
if is_multiple_answer(question):
    ...
    user_answer = st.multiselect("답을 선택하세요:", options, key=f"q_{current_q}")
else:
    user_answer = st.radio("답을 선택하세요:", options, key=f"q_{current_q}")
...
if st.session_state.show_result:
    if st.session_state.answers[current_q]:
        ...
        st.error("틀렸습니다.")
        if is_multiple_answer(question):
            st.error(f'당신의 오답: {", ".join(st.session_state.user_answer)}')
            st.success(
...
with col2:
    if st.button(
        "다음 문제", disabled=(current_q == st.session_state.total_questions - 1)
    ):
        st.session_state.current_question = min(
            st.session_state.total_questions - 1, current_q + 1
        )
        st.session_state.show_result = False
        st.rerun()

# 진행 상황 표시
st.sidebar.progress((current_q + 1) / st.session_state.total_questions)
st.sidebar.write(f"{current_q + 1} / {st.session_state.total_questions} 문제")
